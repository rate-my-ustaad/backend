/**
 * SMME Faculty Scraper
 *
 * - Scrapes faculty names from SMME NUST website
...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			if (token !== env.SCRAPERS_AUTH_TOKEN) {
			...
			const headers = {
				...
				'Referer': 'https://smme.nust.edu.pk/',
				'DNT': '1',
			...
			const response = await fetch('https://smme.nust.edu.pk/faculty/', {
				headers,
				cf: {
					cacheTtl: 0,
